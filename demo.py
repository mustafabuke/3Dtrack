...
def delete_folder_contents(folder_path):
    # Iterate over all the files and folders in the given folder
    for file_or_folder in os.listdir(folder_path):
        ...
        # Check if it's a folder
            ...
            # After deleting the contents, remove the folder itself
            os.rmdir(full_path)
...
def main(args):
    delete_folder_contents("histograms/")
    ...
    while True:
        ...
        frame_name = "/home/mustafabuke/PycharmProjects/image_to_video/mbuke_extract/record2/depth/" + str(2*(frame_id - 1)) + ".png"
        depth_image = cv2.imread(frame_name, -1)
        ...
        for det in dets:
            if det.track_id > 0:
                ...
                depth_of_image = depth_image[int(det.bb_top):int(det.bb_top + det.bb_height), int(det.bb_left):int(det.bb_left + det.bb_width)]
                depth_values = []
                depth_pixels = []
...
parser.add_argument('--video', type=str, default = "demo/mbuke_record2_30fps.mp4", help='video file name')
